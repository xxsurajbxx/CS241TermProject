...
function monteCarloOptiamalStrategySimulation(playersHand, dealerUpcard, deck, count, split=0) {
    ...
    if(val[0]!='21') {
        played=true;
        if(table[val[1]][dealerUpcard][val[0]]['H'][1] < rounds) {action='H'; playersHand.push(deck.draw(count)); result = [header.runStrategy(deck, playersHand, dealerUpcard, count, newStrategy).concat(split>0)];}
