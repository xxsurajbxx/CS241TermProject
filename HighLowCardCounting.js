...
function blackjackSimulation(deck, betAmount) {
    ...
    if(runningCount.value < 0) {
        dealerHand.push(deck.draw(runningCount));
        dealerHand.push(deck.draw(runningCount));
        dealerStrategy(deck, dealerHand, runningCount);
        played = false;
        return 0;
    }
    else {played = true;}
    ...
    //console.log(runningCount.value);
    ...
    if(playerHand[0].value === playerHand[1].value && shouldSplit(playerHand[0].value, dealerHand[0].value)) {
        ...
        var dHandVal = dealerStrategy(deck, dealerHand, runningCount);
